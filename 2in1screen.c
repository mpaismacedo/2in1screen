//gcc -O2 -march=native -mtune=native -D_FORTIFY_SOURCE=2 -fPIE -o 2in1screen 2in1screen.c
...
void rotate_screen(){
	...
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "ELAN Touchscreen", COOR[current_state]);
	system(command);
}
